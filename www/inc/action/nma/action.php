...
//		$url			=$this->GetParam('url'		,'str');
		...
		if($key and $title and $message){
			require_once(dirname(__FILE__).'/class.nma.php');
			$nma = new NotifyMyAndroid();
			$nma_params = array(
					'apikey' => $key,
					'priority' => $priority,
					'application' => $this->conf['app']['name'],
					'event' => $title,
					'description' => $message
			);

			$result=$nma->push( $nma_params);
			if($result){
				$this->DisplayJson(true, array('code'=>200, 'message'=>"Successfully Sent Notification : $title",'api_result'=>$result));
			}
			else{
				$this->DisplayJson(false, array('code'=>500, 'message'=>"Failed to send Notification : $title",'api_result'=>$nma->getError()));				
			}
		}
		else{
